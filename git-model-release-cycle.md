...
Each team environment (K8S namespace) has one YAML file in the [e2ecc-k8s-envs](https://bitbucket.cib.echonet/projects/E2ECC/e2ecc-k8s-envs/) repository, where descriptor changes are made. It lists only the modules under test, with the branch whose image they run; every module not listed is served by the **upstream** environment.
