...
| `RC_YY.X+1` | Next monthly release: created from develop at the cut-off, stabilised for a month | UAT (NRT), STAGING (performance, on demand) / mycreditapp-release |
| `RC_YY.X` | The last release candidate delivered in production; hosts the minor releases | QA-FIX / mycreditapp-minor-release (TRAINING) |
...
RC naming: `RC_YY.n_V0.YYMM.m` (YY year, n incremented on each monthly release, V0.YYMM.m the develop tag the branch is created from). After its go-live, RC_YY.X+1 becomes the RC_YY.X that hosts the minor releases.
...
| RC_YY.X, minor-release branch | `RC_26.7_V0.2607.18` |
| RC_YY.X+1, next major release branch | `RC_26.8_V0.2608.18` |
| Package tags on RC_26.8_V0.2608.18 (next major) (one per bug-fix merge / build) | `branch.increment`, incremented by 1 with no gap: `RC_26.8_V0.2608.18.1` (cut-off), then `RC_26.8_V0.2608.18.2` (bug fix merged); the package freeze creates no tag, it freezes the last tagged package |
| Next production tag | `REL_26.8_V0.2608.18.2` (same increment as the delivered package) |
| Minor release package tag (on RC_26.7_V0.2607.18, at the merge) / production tag (on the RC at go-live) | `RC_26.7_V0.2607.18.22` / `REL_26.7_V0.2607.18.22` [to be confirmed] |
...
- master into RC_YY.X (hotfixes)
- RC_YY.X into RC_YY.X+1 (minor-release fixes and hotfixes)
- RC_YY.X+1 into develop (release fixes, minor-release fixes and hotfixes)
...
On conflict it opens a PR from an intermediate branch `merge_<source>_in_<target>`. Merges into an RC or develop use the **merge commit** strategy (`git merge --no-ff --no-commit`, then `git commit`), never rebase and merge. Into master, the job only fast-forwards (`git merge --ff-only`): no merge commit.
...
| Minor, standard story | Ready for Development → In Progress → Code Review → Testing → Merge → Integration Testing (`RC_YY.X`) |
| Minor, large project | Ready for Development → In Progress → Code Review → Testing → Merge → Integration Testing (epic branch only) → Merge into `RC_YY.X` → Integration Testing (`RC_YY.X`) |
...
- **Integration Testing**: functional tests on the target branch environment (INT (K8S) for develop, TRAINING for RC_YY.X).
...
| When | What |
|---|---|
| Day before the cut-off (monthly only) | Pre cut-off ◆: every merge into develop is blocked, so the package on QA stays stable for the creation of the release branch; the Confluence page of the release is completed with the release details |
...
| After the freeze, before the pre-CRB | Environment promotion (PRE-PROD) *manual*, by APS: rehearsal of the delivery |
| Before the pre-CRB | Pre-release sanity checks in PRE-PROD, to validate the frozen package |
| Tuesday PM | Pre-CRB: evidence (performance, NRT, rehearsal, sanity…) presented to the production teams; if not validated, the release is delayed |
...
| Stabilisation (month M, daily) | Release Preparation (cut-off, month M+1) | Stabilisation (daily) | Release (week n) |
|---|---|---|---|
| Environment promotion (INT) *auto*: end-of-day pre-package deployed on INT | **Pre cut-off** ◆ the day before the cut-off: merges into develop blocked, release Confluence page completed | Environment promotion (UAT) *auto*: NRT | **Package freeze** ◆ |
| Environment promotion (QA) *auto*: same pre-package on QA, when the INT pipeline is successful | **Cut-off** ◆ RC_YY.X+1 created from develop, on the commit of the last package deployed on QA | Environment promotion (STG) *manual, on demand*: performance tests | Environment promotion (PRE-PROD) *manual*, APS |
| | Testing (NRT, Performance): both mandatory | | Sanity checks (PRE-PROD), APS |
| | Bug fixing (regressions, critical bugs): fix branch `fix/E2ECC-…` from RC → Dev Team env → merge into RC → release env | | **Pre-CRB** ■, **CRB** ■, **Go-live**, Sanity checks (PROD), Business Go / NoGo *optional, after the go-live*: validates the opening of a feature hidden behind a toggle |

After the cut-off, only defects and bugs (except Minor) may be merged, into RC_YY.X+1. Anything else needs a justification emailed to Release Management.

### Minor release

Only for topics that cannot wait for the next monthly release. The Jira must be listed on *Hotfix/Minor: Content & Follow-up* before its PR can be merged into RC_YY.X. No performance tests (topics with no performance impact only) and no full NRT: the Development Team runs the NRT on the impacted scope.

| Stabilisation | Release Preparation | Release (week n) | Sign-off (week n) |
|---|---|---|---|
| Environment promotion (minor-release) *auto*: TRAINING | All Jiras closed ◆ *[minor equivalent of the cut-off?]* | **Package freeze** ◆ | **Go-live** |
| | Environment promotion (QA-FIX) *manual*, by DevOps | Environment promotion (PRE-PROD) *manual*, APS | Sanity checks (PROD) |
| | Testing (NRT on impacted scope), Development Team | Sanity checks (PRE-PROD), APS | |
| | Sanity checks (QA-FIX), Development Team | | |
...
|---|---|
| Environment promotion (QA-FIX) *manual* | Environment promotion (PRE-PROD) *manual*, APS |
| Testing (NRT), Development Team | Sanity checks (PRE-PROD) |
| | **Go-live**: new tag from master |
| | Sanity checks (PROD) |

No release branch is created: the fix goes from a feature branch into master, then Git-tooling retrofits it into RC_YY.X, RC_YY.X+1 (if open) and develop.
...
gitGraph
  ...
  checkout main
  merge RC_26.8_V0.2608.18 id: "git-tooling fast-forward (--ff-only)"
```

*Here `main` stands for `master`. The graph starts from the version in production (tagged on master) and the current state of develop. After the go-live, RC_26.8_V0.2608.18 becomes the RC_YY.X that hosts the minor releases.*
...
- **Gate**: a freeze or a decision (pre cut-off, cut-off, package freeze). The package freeze comes before the pre-CRB and creates no tag: it freezes the last tagged package.
...
- **CI/CD**: automation from the DevOps tooling: environment promotions (automatic or manual deploys) and Git-tooling retrofit merges.
...
Git-tooling is one job that runs at fixed times. Each run merges one source branch into one target branch: with a merge commit (`git merge --no-ff --no-commit`, then `git commit`), except into master, which only fast-forwards (`git merge --ff-only`, no merge commit).
...
| 05:00 | Delivered RC (`RC_YY.X` or `RC_YY.X+1`) → `master` | The released code, only once the production package is delivered. Fast-forward only: no merge commit |
...
- **Hotfix**: PR `fix/E2ECC-105402` → master; go-live from master (`REL_26.7_V0.2607.18.21.1`); 19:00 master → RC_26.7_V0.2607.18; 20:00 RC_26.7_V0.2607.18 → RC_26.8_V0.2608.18; 21:00 RC_26.8_V0.2608.18 → develop.
- **Minor**: PR `fix/E2ECC-105310` → RC_26.7_V0.2607.18; 19:00 no commit; 20:00 RC_26.7_V0.2607.18 → RC_26.8_V0.2608.18; 21:00 RC_26.8_V0.2608.18 → develop; go-live (`REL_26.7_V0.2607.18.22`); 05:00 RC_26.7_V0.2607.18 → master.
- **Monthly**: PR `fix/E2ECC-105227` → RC_26.8_V0.2608.18; 19:00 and 20:00 no commit; 21:00 RC_26.8_V0.2608.18 → develop; go-live (`REL_26.8_V0.2608.18.2`); 05:00 RC_26.8_V0.2608.18 → master; RC_26.8_V0.2608.18 becomes the production branch.

### 3 · Good to know

- **Merge commits, no tags**: every automerge into an RC or develop is a merge commit; into master, the job only fast-forwards to the delivered commit. Git-tooling never creates tags: package tags come from cut-off and RC merges, REL tags from go-live.
